...
#ifdef _WINDOWS64
static string ToForwardSlashPath(const string& path)
{
...
static bool EndsWith(const string& value, const char* suffix)
{
	size_t suffixLen = 0;
...
static bool IsSourcesAssetPathPrefix(const string& lowerPath)
{
	return StartsWith(lowerPath, "common/") ||
		StartsWith(lowerPath, "windows64media/") ||
		StartsWith(lowerPath, "minecraft/") ||
		StartsWith(lowerPath, "music/") ||
		StartsWith(lowerPath, "durango/sound/");
}

static string GetSourcesAssetCategory(const string& lowerPath)
{
	if(lowerPath.find("/loc/") != string::npos || EndsWith(lowerPath, ".loc"))
	{
		return "localization";
	}

	if(EndsWith(lowerPath, ".ogg") || EndsWith(lowerPath, ".wav") || EndsWith(lowerPath, ".binka") ||
		EndsWith(lowerPath, ".xwb") || EndsWith(lowerPath, ".xsb") || EndsWith(lowerPath, ".xgs") ||
		EndsWith(lowerPath, ".msscmp"))
	{
		return "sounds";
	}
	...
	if(EndsWith(lowerPath, ".swf") || EndsWith(lowerPath, ".xui"))
	{
		return "ui";
	}

	if(EndsWith(lowerPath, ".pck") || EndsWith(lowerPath, ".arc") || EndsWith(lowerPath, ".xzp") ||
		EndsWith(lowerPath, ".abc") || EndsWith(lowerPath, ".grf") || EndsWith(lowerPath, ".spa"))
	{
		return "packages";
	}

	if(EndsWith(lowerPath, ".ttf") || EndsWith(lowerPath, ".ttc"))
	{
		return "fonts";
	}

	if(EndsWith(lowerPath, ".xml") || EndsWith(lowerPath, ".txt") || EndsWith(lowerPath, ".lang") ||
		EndsWith(lowerPath, ".gameconfig") || EndsWith(lowerPath, ".col") || EndsWith(lowerPath, ".ms"))
	{
		return "config";
	}
...
static bool ResolveMountedOrLocalPath(const string& path, string& resolvedPath)
{
	string mountedPath = StorageManager.GetMountedPath(path.c_str());
	if(mountedPath.size() > 0 && PathExists(mountedPath))
	{
...
static bool ResolveSourcesAssetPath(const string& originalPath, string& resolvedPath)
{
	if(originalPath.empty() || IsAbsolutePath(originalPath))
	{
		return false;
	}

	string normalized = ToForwardSlashPath(originalPath);
	string lower = ToLowerAscii(normalized);

	if(StartsWith(lower, "sources/"))
	{
		return false;
	}

	if(!IsSourcesAssetPathPrefix(lower))
	{
		return false;
	}

	string category = GetSourcesAssetCategory(lower);
	if(category.empty())
	{
		return false;
	}

	string sourcePath = "sources/" + category + "/" + normalized;
	static const char* relativePrefixes[] = { "", "../", "../../", "../../../" };
	for(size_t i = 0; i < sizeof(relativePrefixes) / sizeof(relativePrefixes[0]); ++i)
	{
		string candidate = string(relativePrefixes[i]) + sourcePath;
		if(ResolveMountedOrLocalPath(candidate, resolvedPath))
		{
			return true;
		}
	}
	...
	{
		string cwdPath = ToForwardSlashPath(cwd);
		...
		for(size_t i = 0; i < sizeof(relativePrefixes) / sizeof(relativePrefixes[0]); ++i)
		{
			string candidate = cwdPath + relativePrefixes[i] + sourcePath;
			if(ResolveMountedOrLocalPath(candidate, resolvedPath))
			{
				return true;
			}
	...
	{
		string moduleDir = ToForwardSlashPath(modulePath);
		...
		if(slashPos != string::npos)
		{
			...
			for(size_t i = 0; i < sizeof(relativePrefixes) / sizeof(relativePrefixes[0]); ++i)
			{
				string candidate = moduleDir + relativePrefixes[i] + sourcePath;
				if(ResolveMountedOrLocalPath(candidate, resolvedPath))
				{
					return true;
				}
...
File::File( const wstring& pathname ) //: parent( NULL )
{
	...
	string finalPath;
	if(!ResolveSourcesAssetPath(path, finalPath))
	{
		finalPath = StorageManager.GetMountedPath(path.c_str());
		if(finalPath.size() == 0)
		{
			finalPath = path;
		}
